    ...
    <div className="flex-1 flex flex-col px-4 sm:px-8 py-8 sm:py-12 max-w-6xl mx-auto w-full space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 border-b border-slate-800 pb-6">
        ...
        <input
          ...
          placeholder="Rechercher (ex: Voyante, Sorcière, Loup...)"
          className="bg-[#10141f] border border-slate-700 rounded-xl px-4 py-2.5 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-amber-500 font-mono w-full sm:w-80 shadow-inner"
        />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Liste cliquable de toutes les cartes */}
            ...
            return (
              <button
                ...
                className={`w-full p-3.5 rounded-2xl border text-left transition-all flex items-center justify-between cursor-pointer ${
                  isSelected 
                    ? 'bg-[#181f30] border-slate-400 text-white shadow-xl scale-[1.02]' 
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 shrink-0 bg-black/60 rounded-xl border border-white/10 p-1 flex items-center justify-center shadow">
        ...
        {/* Détail du Rôle Sélectionné avec Grand Artwork & Description */}
        <div 
          className="lg:col-span-7 bg-[#10141f] border-2 rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl flex flex-col md:flex-row items-center md:items-start gap-6"
          style={{ borderColor: activeRole.color }}
        >
          <div className="shrink-0 flex flex-col items-center gap-3">
            <div className="w-48 h-48 bg-gradient-to-b from-slate-900 via-slate-950 to-black rounded-2xl border-2 border-white/15 p-4 flex items-center justify-center shadow-2xl">
